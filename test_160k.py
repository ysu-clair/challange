...
image_datasets['query_street'] = CustomData160k_drone( os.path.join(data_dir,'workshop_query_street') ,data_transforms, query_name ='/home/ubuntu-user/LXY/University1652-Baseline/data/test/workshop_query_street/query_street_name.txt')
...
query_lable,query_path=get_SatId_160k(query_path)
...
if __name__ == "__main__":
    ...
    query_lable=np.array(query_lable)
...
#result = {'gallery_f':gallery_feature.numpy(),'gallery_label':gallery_label,'query_f':query_feature.numpy(),'query_label':query_lable}
# 将张量从 GPU 移动到 CPU，然后转换为 NumPy 数组
result = {
    'gallery_f': gallery_feature.cpu().numpy(),
    'gallery_label': gallery_label,
    'query_f': query_feature.cpu().numpy(),
    'query_label': query_lable
}
 检查查询集和图库集中是否有共同的标签
common_labels = np.intersect1d(query_lable, gallery_label)
